...
export class TilingGenerator {
    ...
    async generateFocalTiles(images, tiles, allowImageRepetition, useDramaticScaling) {
        ...
        if (useDramaticScaling) {
            ...
            minScale = 0.4;
            maxScale = 3.0; // Increased from 2.2 to allow for even larger tiles
        } else {
            // Uniform scaling - less variation
            minScale = 0.6;
            maxScale = 2.5; // Increased from 1.8 to allow for even larger tiles
        }
    ...
    async generateFieldTiles(images, tiles, allowImageRepetition, useDramaticScaling) {
        ...
        const baseSizeMultiplier = useDramaticScaling ? 4 : 6; // Reduced from 5/7 to allow for even larger tiles
