#!/usr/bin/env node
/**
 * Final Fixed Upload Script - With all correct constraint values
 * 
...
async function processAllImages() {
    console.log('=== Final Fixed Image Upload Pipeline ===\n');
    ...
    // First, let's check a sample record to see what values work
    console.log('Checking database for valid field values...');
    const { data: sampleData } = await supabase
        .from('images')
        .select('*')
        .eq('provider', 'cms')
        .not('palette_suitability', 'is', null)
        .limit(1);
        
    let validPaletteSuitability = 'vibrant'; // default
    if (sampleData && sampleData.length > 0) {
        validPaletteSuitability = sampleData[0].palette_suitability;
        console.log(`Using palette_suitability value: ${validPaletteSuitability}`);
    }
    ...
    for (let i = 0; i < imageFiles.length; i++) {
        ...
        try {
            ...
            const imageRecord = {
                ...
                is_photograph: false,
                white_edge_score: 0,
                palette_suitability: validPaletteSuitability,
                rotation: 0,
            ...
            if (i < imageFiles.length - 1) {
                const waitTime = openai ? 2000 : 100; // Wait less if not using AI
            ...
            // If we get a constraint error, try to understand it better
            if (error.message && error.message.includes('violates check constraint')) {
                console.log('   Debugging: Check your database constraints');
                console.log('   Run: node scripts/check_palette_suitability.js');
            }
        }
    }
