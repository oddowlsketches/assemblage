...
def generate_metadata(image_path):
    ...
    try:
        ...
        model_names = ["gpt-4o", "gpt-4o-vision", "gpt-4-vision"]
        last_error = None
        ...
        for model_name in model_names:
            try:
                ...
                # Make the API request using the older API format
                ...
                if "DESCRIPTION:" not in content or "TAGS:" not in content:
                    raise ValueError("Response does not contain expected format")
                    
                description = content.split("DESCRIPTION:")[1].split("TAGS:")[0].strip()
                tags_text = content.split("TAGS:")[1].strip()
                tags = [tag.strip().strip('[]').strip('"').strip("'") for tag in tags_text.split(',')]
                tags = [tag for tag in tags if tag]  # Remove empty tags
                
                if not description or not tags or len(tags) < 5:
                    raise ValueError("Invalid response: missing description or tags")
