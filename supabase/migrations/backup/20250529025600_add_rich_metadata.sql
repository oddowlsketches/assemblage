...
DO $ 
BEGIN
  -- Add check_image_role constraint
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_image_role'
  ) THEN
    ALTER TABLE public.images 
    ADD CONSTRAINT check_image_role 
    CHECK (image_role IN ('texture','narrative','conceptual'));
  END IF;
  
  -- Add check_palette_suitability constraint
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_palette_suitability'
  ) THEN
    ALTER TABLE public.images 
    ADD CONSTRAINT check_palette_suitability 
    CHECK (palette_suitability IN ('vibrant','neutral','earthtone','muted','pastel'));
  END IF;
  
  -- Add check_metadata_status constraint
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_metadata_status'
  ) THEN
    ALTER TABLE public.images 
